...
class MovielensToPostgresOperator(BaseOperator):
    ...
    def execute(self, context):
        with MovielensHook(self._movielens_conn_id) as movielens_hook:
            ratings = list(movielens_hook.get_ratins(
                start_date=self._start_date,
                end_date=self._end_date)
            )

        postgres_hook = PostgressHook(
            postgres_conn_id=self._postgres_conn_id
        )

        insert_queryes = [
            self._insert_query.format(",".join([str(_[1]) for _ in sorted(ratings.items())])) for rating in ratings
        ]
